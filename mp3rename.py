...
from types import Tuple, Object, List
import logging
import os
import sys
import argparse
...
DESCRIPTION: str = """
    The main task of this script is to rename mp3 files using random names.
"""
    ...
    if os.path.isdir(args.folder) and args.mode in MODES:
        ...
        if list_of_files:
            ...
            if args.mode == MODES[0]:
                for old_file in list_of_files:
                    ...
                    try:
                        ...
                        logging.info("File {} has been renamed to {}".format(
                            old_file, new_name
                        ))
                    except Exception as error:
                        logging.error(
                            "Could not rename file {} using {} mode.".format(
                                old_file, args.mode
                            )
                        )
                        logging.error(error)
            # ByCode
                ...
                new_name: str = ""
                for old_file in list_of_files:
                    for symbol in os.path.splitext(
